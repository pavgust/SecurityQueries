...
predicate edges_internal(FunctionCall c1, FunctionCall c2) {
  c1.getTarget() = c2.getEnclosingFunction() or
  c1.getTarget() = c2.getEnclosingFunction().(MemberFunction).getAnOverriddenFunction()
}

import Exploration
import Exploration::ImportantTypes
predicate isSource(FunctionCall call) {
  call.getEnclosingFunction().(GsOperator).isVisible()
}

predicate isSink(FunctionCall call) {
  call.getTarget().getQualifiedName() = "scan_comment" // Customize destination function here
}
