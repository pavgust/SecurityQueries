...
from
  DataFlow::Node source, DataFlow::Node sink, DataFlow::Node readValue,
  DataFlow::Node typedReadQualifier, FieldAccess typedRead //VariableAccess val, FieldAccess typedRead
where
  any(OpStackCollectionFetch c).hasFlow(source, sink) and
  any(CollectionFetchFunction f).reads(sink.asExpr(), readValue.asDefiningArgument()) and
  ...
  any(GsOperator g | g.isVisible()).getACalledFunction(_) = source.getFunction() and
...
select sink, sink + " flows from $@, and its content is read as $@.", source, source.toString(), typedRead,
  typedRead.toString()
